...
class CommentsUserController extends Controller
{
    public function kasihratinguser(Request $request, $id_user) {
        $tukang = auth()->user();
        ...
        $rating = RatingUserModel::where('id_user', $id_user)
    ...
    public function kasihulasanuser(Request $request, $id_user)
    {
        ...
        if (!$tukang->id_tukang) {
            return response()->json([
                'status' => 'error',
                'message' => 'Hanya tukang yang dapat memberi ulasan.',
            ], 403); 
        }
    ...
    public function getKomentarUser($id_user)
    {
        $tukang = auth()->user();
        ...
        $ratings = RatingUserModel::where('id_user', $id_user)
            ->with(['tukang:id_tukang,name,foto_diri']) // Relasi ke model TukangModel
            ->get();
        ...
        $response = $ratings->map(function ($rating) {
            return [
                'id_tukang' => $rating->id_tukang,
                'tukang_name' => $rating->tukang->name,
                'tukang_foto' => $rating->tukang->foto_diri ? 'data:image/png;base64,' . base64_encode($rating->tukang->foto_diri) : null,
